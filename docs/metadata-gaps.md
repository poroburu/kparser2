...
|----|---------------------------------|----------------|----------------|
| `local-player-name` | Slice may lack `0x00A` / `0x00D`; `0x00DF` has id but not the name | kpacket `hello.player_name`; NDJSON session header `player_name`; `EntityRegistry.registerLocalPlayerName` (pending name is visible to `localPlayerName` before UniqueNo) | Login `0x00A` name@132, PC `0x00D` name when `SendFlg.Name`, or say-copy `0x18`/`0x19` on the same capture |
