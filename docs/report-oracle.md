...
- **2026-09-19** — Offline loop smoke: heat skip + `report fights` / `report offense` on `bst_camp_multi`. Live `probe` was down (plugin offline); wire tick untested. Dual capture now a hard gate (do not unobserved-skip). Ingest family draft [PR #15](https://github.com/poroburu/kparser2/pull/15) (spikes, prepare starts, cmd-5 ItemUses, msg-20 `Unknown`, drain Recovery; unused combat fixtures attached to `DetailedReports`). [#4](https://github.com/poroburu/kparser2/issues/4) classified matrix posted; [#9](https://github.com/poroburu/kparser2/issues/9) freeze lifted for existing compare only. Next live thread: attach kparser RAM capture, then heat/classify.
